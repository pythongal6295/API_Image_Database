...
class PictureTests(TestCase):
    ...
    def setUp(self):
        self.db = DataSet('sqlite:///:memory:')
        self.base_directory = 'C:\\Users\\kelly_kjenkz1\\UW_Python_320A\\Lesson_10\\assignment-10-pythongal6295'
    ...
    #Test for list user images that an incorrect user does not create a list
    #Test for receoncile images that it finds discrepancies in both places
    #Test for reconcile images that returns False if there is no user_id

    def test_add_picture(self):
        '''tests a picture's data is added to the database
        '''

        picture_id = main.picture_id('daisy45', self.picture)
        return_value = main.add_picture(user_id='daisy45', tags='#fighting #Zephyr', picture_id=picture_id,
    ...
    def test_add_picture_multiple_pics(self):
        ...
        '''
        picture_id1 = main.picture_id('daisy45', self.picture)
        main.add_picture(user_id='daisy45', tags='#badguys #inhuman', picture_id=picture_id1,
            picture_collection=self.picture, user_collection=self.users)

        picture_id2 = main.picture_id('daisy45', self.picture)
        main.add_picture(user_id='daisy45', tags='#friends #family', picture_id=picture_id2,
            picture_collection=self.picture, user_collection=self.users)

        picture_id3 = main.picture_id('daisy45', self.picture)
        main.add_picture(user_id='daisy45', tags='#badguys #Zephyr', picture_id=picture_id3,
            picture_collection=self.picture, user_collection=self.users)
    ...
    def test_add_picture_nonexistant_user_id(self):
        ...
        '''
        picture_id = main.picture_id('test_11', self.picture)
        return_value = main.add_picture(user_id='test_11', tags='#badguys #inhuman', 
            picture_id=picture_id, picture_collection=self.picture, user_collection=self.users)
    ...
    def test_length_checker_tags(self):
        ...
        picture_id = main.picture_id('daisy45', self.picture)
        return_value = main.add_picture(user_id='daisy45', tags=tags, 
            picture_id=picture_id, picture_collection=self.picture, user_collection=self.users)
    ...
    def test_save_picture(self):
        ...
        path = os.path.join('daisy45', *tag_lst)

        #print(path)

        picture_id = main.picture_id('daisy45', self.picture)

        main.save_picture('daisy45', picture_id, tag_lst, self.base_directory, self.picture)

        assert os.path.exists(self.base_directory + '\\images\\daisy45\\adventure\\cars\\saturday\\000001.png')

    #Test for list user images that the correct tuples were created
    def test_list_user_images(self):
        '''
        ensures a list of user images is created and formatted correctly
        '''
        picture_id1 = main.picture_id('jemma32', self.picture)
        # main.add_picture(user_id='jemma32', tags='#science #biology', picture_id=picture_id1,
        #     picture_collection=self.picture, user_collection=self.users)
        tag_lst = main.tag_path(tags='#science #biology')
        main.save_picture('jemma32', picture_id1, tag_lst, self.base_directory, self.picture)

        picture_id2 = main.picture_id('jemma32', self.picture)
        # main.add_picture(user_id='jemma32', tags='#friends #family', picture_id=picture_id2,
            # picture_collection=self.picture, user_collection=self.users)
        tag_lst = main.tag_path(tags='#friends #family')
        main.save_picture('jemma32', picture_id2, tag_lst, self.base_directory, self.picture)

        pic_lst = main.list_user_images('jemma32', self.base_directory)

        assert pic_lst == [('jemma32', 
            'C:\\Users\\kelly_kjenkz1\\UW_Python_320A\\Lesson_10\\assignment-10-pythongal6295\\images\\jemma32\\biology\\science', '0000000001.png'), 
            ('jemma32', 
            'C:\\Users\\kelly_kjenkz1\\UW_Python_320A\\Lesson_10\\assignment-10-pythongal6295\\images\\jemma32\\family\\friends', '0000000001.png')]



class APITests(flask_unittest.AppTestCase):
    ...
    def create_app(self):
        ...
        api.add_resource(apipy.Home, '/')
        api.add_resource(apipy.Users, '/users')
        api.add_resource(apipy.Images, '/images')
        api.add_resource(apipy.Differences, '/differences')
    ...
    #I am not sure how to test that Flask and the webpages are running
    #correctly besides just running api.py

    #Currently getting, TypeError: Expected create_app to return a Flask object

    def test_users(self, app):

        with app.test_client() as client:
            rv = client.get('/users')
        #Was trying to get all the way through the test to this generic assert, but never got there
            self.assertInResponse(rv, {'users': []})
